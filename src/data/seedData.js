...
const lessonOverrides = {
  ...
  'habit-identity': {
    ...
    breakDown: [
      'The problem is plans that collapse because they demand more willpower than an ordinary day can provide.',
      'The signal is reset cycles: ambitious plan → short compliance → environment wins → shame → next ambitious plan.',
      'The first move is to shrink the behavior until it can survive a bad day.',
      'Good practice looks like: attach a two-minute rep to an existing cue and run it for seven days before adding more.',
      'The common mistake is designing habits for a better version of yourself that does not yet exist.',
      'Avoid using habit language to judge people whose circumstances require structural support.',
    ],
    remember: [
      'Tiny reps are identity votes.',
      'Environment shapes behavior more reliably than motivation.',
      'A cue that already exists makes the habit easier to start.',
      'Character is built by repetition, not willpower spikes.',
    ],
    questions: [
      { type: 'Recall', prompt: 'What is the difference between motivation-based habits and identity-based habits?' },
      { type: 'Scenario', prompt: 'Someone you mentor says they keep failing to build a reading habit. What is the first thing you ask or change?' },
      { type: 'Judgment', prompt: 'When might habit advice be the wrong tool for someone struggling to change behavior?' },
      { type: 'Reflection', prompt: 'Which small repeated action has most shaped who you are becoming?' },
    ],
  },
};
...
const leadershipLessons = lessonSpecs.map((spec, index) => {
  ...
  ...lesson,
  quickVersion: lessonOverrides[lesson.slug]?.quickVersion ?? [],
  articleParagraphs: buildLessonArticle(lesson),
  breakDown: lessonOverrides[lesson.slug]?.breakDown ?? [],
  remember: lessonOverrides[lesson.slug]?.remember ?? [],
  questions: lessonOverrides[lesson.slug]?.questions ?? [],
